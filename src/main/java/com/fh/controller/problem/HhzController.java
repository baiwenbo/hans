...
import com.fh.service.hans.HhzService;
 ...
 */
@Controller
@RequestMapping(value="/hhz")
public class HhzController extends BaseController {
	...
	@Resource(name="hhzService")
	private HhzService qizheService;
	...
	public String saveAudit(HttpSession session) throws Exception {
		...
		return "redirect:/hhz/auditPage.do?projectName="+pd.get("projectName")+"&ID="+pd.get("scpcid")+"";
	...
	public ModelAndView goImageAdd(Integer id, String projectName) {
		ModelAndView mv = this.getModelAndView();
		mv.setViewName("hhz/imageAddPage");
		mv.addObject("id", id);
	...
	public ModelAndView getImageDeal() throws Exception {
		...
		mv.setViewName("hhz/imageDealPage");
		mv.addObject("varList", varList);
		mv.addObject("pd", pd);
		return mv;
