...
$.fn.html5uploader = function(opts){
	...
	var defaults = {
		...
		itemTemplate:'<li id="${fileID}file"><span class="filename">${fileName}</span><br></li>',//上传队列显示的模板,最外层标签使用<li>
		...
		//默认模板
		//itemTemplate:'<li id="${fileID}file"><span class="filename">${fileName}</span><span class="progressnum">0/${fileSize}</span><br><a class="uploadbtn">上传</a><a class="delfilebtn">删除</a></li>',//上传队列显示的模板,最外层标签使用<li>
	...
	this.each(function(){
		var _this = $(this);
		//先添加上file按钮和上传列表
		var inputstr = '<input class="uploadfile" style="visibility:hidden;display:none;" type="file" name="fileselect[]"';
		if(option.multi){
			inputstr += 'multiple';
			}
		inputstr += '/>';
		inputstr += '<a href="javascript:void(0)" class="uploadfilebtn am-btn am-btn-primary am-round am-icon-file-photo-o">';
		...
		//创建文件对象
			  ...
			  onSelect: option.onSelect||function(files){
				 ...
				 //如果配置非自动上传，绑定上传事件
				 if(!option.auto){
					_this.find('.uploadbtn').on('click',function(){
