...
// 	window.location.href = "/carPdf/download?urlPdf="+url+'&orderNo='+orderNo;
//  }
...
*/
 ...
 $.when( 		
 		$.getJSON('/examine/findEvaluationPictures?orderId='+orderId,function (data) {
...
$.when( 
 		$.getJSON('/examine/findCustomerPictures?orderId='+orderId,function (data) {	
 ...
 $.when( 
		 $.getJSON('/examine/findContractPictures?orderId='+orderId,function (data) {
		...
		/*	var number='#dropdown4 input[type="number"]'
			var text='#dropdown4 input[type="text"]'
			var radio='#dropdown4 input[type="radio"]'
			var textarea='#dropdown4 textarea'
 
			$(number).each(function(key,val){
				var name = $(this).attr("name")
				var value =  data.contract[name]
				$(this).val(value)
			})
			$(text).each(function(key,val){
				var name = $(this).attr("name")
				var value =  data.contract[name]
				$(this).val(value)
			})
			...
			$.each(data.contract.contractTypes, function (key, val) {
				var items = []
				if (val.status == 1) {								
				items.push('<option value="' + val.id + '"selected>' + val.contractType + '</option>')
				}
				else {
				items.push('<option value="' + val.id + '">' + val.contractType + '</option>')
				}										
				$('.contractTypes').append(items.join(''))
			}); 
			
			if (data.contract.rightName == '春律') {
				$("#rightName option[value='春律']").attr("selected", true);
			}
			else if (data.contract.rightName == '瑞博恩') {
				$("#rightName option[value='瑞博恩']").attr("selected", true);
			}	
			else if (data.contract.rightName == '蔷薇') {
				$("#rightName option[value='蔷薇']").attr("selected", true);
			}	*/
	})
 ).then(function(){
		//合同照片
