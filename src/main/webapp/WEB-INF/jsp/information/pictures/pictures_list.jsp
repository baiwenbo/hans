  ...
  <div class="row-fluid">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td>
										<td>
										<a href="http://www.chebaiwen.com/cbw/${var.URL}" title="${var.NAME}" class="bwGal"><img src="http://www.chebaiwen.com/cbw/${var.URL}" alt="${var.NAME}" width="100"></a>
										</td>
										<td>${var.ID}</td>
										<td>${var.NAME}</td>
										<td>${var.TYPE}</td>
								...
								<td style="width: 30px;" class="center">
										...
										<div>
											...
											<c:if test="${QX.del == 1 }">
											<li><a style="cursor:pointer;" title="删除" onclick="del('${var.ID}','${var.URL}');" class="tooltip-error" data-rel="tooltip" title="" data-placement="left"><span class="red"><i class="icon-trash"></i></span> </a></li>
											</c:if>
										</div>
								</td>
							</tr>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function del(Id,PATH){
			...
			if(confirm("确定要删除?")){ 
				top.jzts();
				var url = "<%=basePath%>pictures/delete.do?ID="+Id+"&URL="+PATH+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>pictures/goEdit.do?ID='+Id;
			 diag.Width = 600;
			 diag.Height = 465;
			 diag.CancelEvent = function(){ //关闭事件
