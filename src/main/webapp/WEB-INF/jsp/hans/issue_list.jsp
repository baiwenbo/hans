...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="issue/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">问题ID</th>
						<th class="center">创建人</th>
						<th class="center">问题标题</th>
						<th class="center">问题内容</th>
					...
					<c:when test="${not empty varList}">
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center'>${var.TAGS}</td>
								<td class='center'>${var.SOURCE}</td>
								<td class='center'>${var.FAVORITES_RATE}</td>
								<td class='center'>${var.ANSWER_RATE}</td>
								<td class='center'>${var.CREATE_TIME}</td>
								<td style="width: 30px;display: none;" class="center">
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					...
					<c:if test="${QX.del == 1 }">
					<a class="btn btn-small btn-danger" style="display: none;" onclick="makeAll('确定要删除选中的数据吗?');" title="批量删除" ><i class='icon-trash'></i></a>
					</c:if>
				</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>issue/goAdd.do';
			 diag.Width = 600;
			 diag.Height = 500;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>issue/delete.do?ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>issue/goEdit.do?ID='+Id;
			 diag.Width = 600;
			 diag.Height = 500;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>issue/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>issue/excel.do';
		}
